...
def draw_walker(screen, walker, viewport, color=(255, 0, 0)):
...
def main():
    ...
    grid = CityGrid(
        num_streets=6, num_avenues=10,
        street_block_length=20.0, street_crosswalk_length=8.0,
        avenue_block_length=30.0, avenue_crosswalk_length=8.0
    )

    viewport = Viewport(grid.width, grid.height, 800, 800)

    # Walker starting at top-left corner (0,0)
    walker = Walker(start_x=0, start_y=0, speed=50.0, grid=grid)
    ...
    while running:
        ...
        draw_walker(screen, walker, viewport)
